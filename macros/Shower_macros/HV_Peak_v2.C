/* 
   ...
   by executing bbsh_cos_cal.C script. HV_Peak_v2.C can take either 
   ...
   root [4] FitRuns(<desired_peakpos_at_trigger>)
            #peak_pos = -1 => Use in case one wants to give FADC peak positions as input. 
   root [5] WriteHV()  #writes calibrated HV setting for desired peak position
   ----
   M. K. Jones  Created
   P. Datta     Edited
*/
...
void ReadPeak(Int_t nrun) {
  ...
  if (!trigtoFADC_ratio)
    ...
    InFile = Form("Output/run_%d_sh_peak_FADC.txt",nrun);
