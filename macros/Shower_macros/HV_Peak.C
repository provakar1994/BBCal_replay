/* 
   ...
   root [0] .L Shower_macros/HV_Peak.C+
   ...
   *** This script uses a outdated PS channel map and also doesn't
   ...
   to date version is HV_Peak_v2.C .
*/
...
//
void WriteHV() {
  string OutFile = Form("hv_set/hv_updated_sh_%d_%d.set",RunList[0],RunList[RunList.size()-1]);
  ofstream outfile_hv;
...
void ReadPeak(Int_t nrun) {
   string InFile = Form("Output/run_%d_peak.txt",nrun);
