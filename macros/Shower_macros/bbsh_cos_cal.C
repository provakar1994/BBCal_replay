...
void bbsh_cos_cal ( int nrun = 366, int event = -1 ){
  ...
  int hADCamp_bin = 120, hamptointratio_bin = 16;
  double hADCamp_min = 0., hamptointratio_min = 1.; 
  double hADCamp_max = 60., hamptointratio_max = 3.;

  // Take user inputs
  cout << " Run number? " << endl;
  cin >> nrun;
  shgui::runLabel->SetText(TString::Format("Run %d",nrun));
  ...
  if(histo_limits){
    cout << " nbins? min? max? [Default: nbins=120, min=0., max=60.] " << endl;
