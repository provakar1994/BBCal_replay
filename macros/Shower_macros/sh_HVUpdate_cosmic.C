/*
  This macro generates calibrated HV for BB Shower.
  ----
...
void sh_HVUpdate_cosmic(Int_t nrun=0, Double_t des_TrigAmp=15., 
			bool userInput=0) {
  if(userInput){
    multiRun = 1;
  ...
  for (Int_t nc=0;nc<2;nc++) {
	...
	if (HV_Block[nc][ns][nch] == -1 ) {
	  ...
	  if(fabs(HV_new)>2400 || (HV_new != HV_new)){
  ...
  h_hv_xy_new->GetZaxis()->SetRangeUser(-2400.,-800.);
  c1->cd(2);
  h_hv_xy_old->Draw("text col");
  // h_hv_xy_old->SetMarkerSize(1.8);
  h_hv_xy_old->GetZaxis()->SetRangeUser(-2400.,-800.);
  c2->cd();
