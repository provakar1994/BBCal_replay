/*
  ...
  be bbps_cos_cal.C macro) for visual comparison of any shift in the peak. To execute, do:
  -----
  [a-onl@aonl2 macros]$ root -l
  root [0] .x PreShower_macros/ps_cospeak_overlay.C(<nrun1>,<nrun2>)
  -----
...
void ps_cospeak_overlay(int nrun1 = 100, int nrun2 = 200){

  gErrorIgnoreLevel = kError; // Ignores all ROOT warnings 

  shgui::SetupGUI();

  TFile *f1 = new TFile(Form("hist/run_%d_ps_peak_Trigger.root",nrun1),"READ");
  TFile *f2 = new TFile(Form("hist/run_%d_ps_peak_Trigger.root",nrun2),"READ");

  TString OutF_peaks;
  OutF_peaks = Form("plots/PS_cos_run_overlay_%d_and_%d.pdf",nrun1,nrun2);
