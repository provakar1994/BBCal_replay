...
double Pars[3]={0.};
double ParErrs[3]={0.};
double trigTofadc_ratiosPS[kNrows*kNcols]={0.};
...
void bbps_cos_cal ( int nrun=366, int event=-1, bool userInput=1 ){
  ...
  if(!T) { 
    ...
    for(int rn=0; rn<nRuns; rn++){
      ...
      // 				  runList[rn],eventsReplayed[rn]); // use this for runs replayed during GMn time
      T->Add(filename);
      T->Add(filename_seg);
    }    
  ...
  for(int r=0; r<kNrows; r++){
	...
	//   lowerBinC = hADCamp_min + (maxBin)*binWidth - (1.2*Pars[2]);
	//   upperBinC = hADCamp_min + (maxBin)*binWidth + (2.0*Pars[2]);
	// }else if( r==1&&c==1 ){
	//   lowerBinC = hADCamp_min + (maxBin)*binWidth - (1.0*Pars[2]);
	//   upperBinC = hADCamp_min + (maxBin)*binWidth + (2.0*Pars[2]);
	// }else{
	//   lowerBinC = hADCamp_min + (maxBin)*binWidth - (2.5*Pars[2]);
	//   upperBinC = hADCamp_min + (maxBin)*binWidth + (2.8*Pars[2]);
	// } 
	//for 500k events
	if( r==0 || r==(kNrows-1)){ // To make the fits better for top and bottom rows
	  lowerBinC = hADCamp_min + (maxBin)*binWidth - (1.2*Pars[2]);
	  ...
	  upperBinC = hADCamp_min + (maxBin)*binWidth + (2.0*Pars[2]);
	}// else if( (r==15&&c==0) ){ //|| (r==20&&c==1) ){
	//   lowerBinC = hADCamp_min + (maxBin)*binWidth - (0.8*Pars[2]);
	//   upperBinC = hADCamp_min + (maxBin)*binWidth + (1.0*Pars[2]);
	// }
	else{
	  lowerBinC = hADCamp_min + (maxBin)*binWidth - (1.8*Pars[2]);
	  upperBinC = hADCamp_min + (maxBin)*binWidth + (2.2*Pars[2]);
	} 
	//for <500k events
	...
	NinPeakPS = hADCamp[r][c]->Integral( hADCamp[r][c]->FindFixBin(lowerBinC), 
	...
	HVcorrection = 1.0;
      ...
      fitData.width(12); fitData << NinPeakPS;
      fitData.width(12); fitData << Flag << endl;

      outfile_data << Pars[1] << " "   << ParErrs[1] << " " << endl; 
...
void processEvent( int entry, bool trigAmp ){
  ...
  for(r = 0; r < kNrows; r++) {
	...
	if( r==0 ){ // Bottom row     
	  ...
	  if( (gPulse[r][c+1]==true&&gPulse[r+2][c+1]==true) ){ //&&(gPulse[r-1][c+1]==true&&gPulse[r+3][c+1]==true)&&(gPulse[r+1][c+2]==false) ){
	    if(trigAmp) {
	      hADCamp[r][c]->Fill( trigTofadc_ratiosPS[m]*adc_amp[r][c] ); 
	    }else{
	      hADCamp[r][c]->Fill( adc_amp[r][c] );
...
void makeSummaryPlots( string runnumber, string date, bool trigAmp = 0 ){
  ...
  for( int i = 0; i < 3; i++ ){
    ...
    // 	Gr[i]->SetTitle( Form("Run# %d | HV Correction Factor vs Block No. for PS blocks | %s",nrun,date.c_str()) );
    // 	Gr[i]->GetXaxis()->SetTitle("Block Number");
...
void GetTrigtoFADCratio(){
  ...
  if (infile_data.is_open() ) {
	...
	trigTofadc_ratiosPS[elemID] = theRatio;
      }
      delete tokens;
    }
    infile_data.close();
