/*
  ...
  This script plots the ADC time for individual PS blocks, fits them using Gaussian and
...
// Main
void ps_atime_align_cosmic (const char *configfilename){
  ...
  if(!Multiple_runs){
    OutPeaks = Form("plots/ps_atime_align_cosmic_%d.pdf",fst_run);
    outFile = Form("hist/ps_atime_align_cosmic_%d.root",fst_run);
    outAtimeOffset = Form("Output/ps_atime_cosmic_offset_target_%0.1fns_%d.txt",target_atime,fst_run);
  }else{
    OutPeaks = Form("plots/ps_atime_align_cosmic_%d_%d.pdf",fst_run,lst_run);
    outFile = Form("hist/ps_atime_align_cosmic_%d_%d.root",fst_run,lst_run);
    outAtimeOffset = Form("Output/ps_atime_cosmic_offset_target_%0.1fns_%d_%d.txt",target_atime,fst_run,lst_run);
  }
  ...
  fout->cd();
  TH1F *h_atimePeakpos = new TH1F("h_atimePeakpos","ADC time peak pos (ns) vs PS blocks; PS blocks",kNblks,0,kNblks);
  h_atimePeakpos->GetYaxis()->SetRangeUser(0.,0.2);
...
void processEvent( int entry = -1 ){
  ...
  int nhit = T->Ndata_bb_ps_a_time;
  for (int ihit=0; ihit<nhit; ihit++) {
    int psrow = T->bb_ps_adcrow[ihit];
    int pscol = T->bb_ps_adccol[ihit];
    double atime = T->bb_ps_a_time[ihit];
    h_atime[psrow][pscol]->Fill(atime);
  }
