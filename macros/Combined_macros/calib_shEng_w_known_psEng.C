/*
  ...
  root [0] .L Combined_macros/calib_shEng_w_known_psEng.C+
...
const Int_t ncell = 189;   // total Shower blocks.
...
void calib_shEng_w_known_psEng(const char *configfilename, Int_t iter=1)
{
  ...
  while( T->GetEntry( elist->GetEntry(nevent++) ) ){
    ...
    // Idea is to freeze the PS calibration constants. So..
    E_e = E_e - ClusEngPS_mod;
